...
*   **AI 智能出题**：
    *   **参数配置**：支持选择科目、题型、难度、数量和自定义主题。
    *   **深度思考**：提供“启用深度思考”开关（默认开启），允许 AI 进行深度推理以提高题目质量。
    *   **实时预览**：生成的题目会实时渲染在右侧预览面板。
    *   **历史记录**：自动保存出题记录，可随时查看和回溯。
*   **用户体验**：
    *   响应式布局。
    *   加载状态提示。
    *   错误处理与提示。
...
## 4. Web 前端 (Vue 3 重构版) (`novi-vue-app`)

`novi-vue-app` 是基于 Vue 3 生态重构的现代化前端应用，旨在提供更流畅、可维护的交互体验。
...
*   **路由管理**：Vue Router
*   **HTTP客户端**：Axios
*   **UI 框架**：Bootstrap 5 + Bootstrap Icons / Sass
*   **工具库**：Marked.js (Markdown渲染), Highlight.js (代码高亮)

### 4.2 核心特性

*   **组件化架构**：将聊天、设置、出题等功能模块化，便于维护和复用。
*   **增强的 UI/UX**：
    *   **打字机效果**：优化的 AI 回复打字机动画，支持光标闪烁跟随。
    *   **平滑过渡**：聊天气泡、侧边栏切换等均包含流畅的过渡动画。
    *   **响应式侧边栏**：支持折叠/展开，适配不同屏幕尺寸。
*   **状态管理**：使用 Pinia 集中管理用户认证、聊天记录、应用主题等状态。
*   **现代化开发体验**：支持热重载 (HMR)、环境变量配置等。
...
src/
├── api/             # API 接口封装
├── assets/          # 静态资源 (SCSS, Images)
├── components/      # Vue 组件
│   ├── chat/        # 聊天相关组件 (ChatInput, ChatMessage)
│   ├── common/      # 通用组件 (TypeWriterVal)
